...
model = dict(
    ...
    tracker=dict(
        ...
        obj_score_thr=0.5, 
        regression=dict(
            obj_score_thr=0.0000001,
            nms=dict(type='nms', iou_threshold=0.3), #0.3
            match_iou_thr=0.3), #0.3
        reid=dict(
            num_samples=15, #10
            img_scale=(256, 128),
            img_norm_cfg=None,
            match_score_thr=3.0, #3.0
            match_iou_thr=0.1), #0.1
        momentums=None,
        num_frames_retain=15)) #10
